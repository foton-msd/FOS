...
class FmpiVqirApp(models.TransientModel):
    _name = "fmpi.vqir.app"
    _description = "FMPI - VQIR Approval Notes"

    name = fields.Text(string="Approval Notes")
    ...
    def action_app(self):
        ...
        if fmpi_vqir_obj.vqir_state == 'ack':
            vqir_state_logs = "Document:" + fmpi_vqir_obj.name + "\n" + \
                ...
                "Approval notes: " + (self.name or '') + "\n" + \
                "--------------------------------------------------\n"
            fmpi_vqir_obj.write({'vqir_state': 'approved',
                'vqir_state_logs': vqir_state_logs + (fmpi_vqir_obj.vqir_state_logs or '')})
            fmpi_vqir_obj.action_app_api()
        else:
