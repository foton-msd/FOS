...
class FotonUnits(models.Model):
  ...
  @api.model_cr
  def init(self):
    tools.drop_view_if_exists(self.env.cr, "view_fas_fu")
    self.env.cr.execute("""
      CREATE OR REPLACE VIEW view_fas_fu AS
      SELECT * FROM one_fu WHERE fmpi_dealer_id = 
      (SELECT dealer_id FROM res_company LIMIT 1);
      """)
FotonUnits()
