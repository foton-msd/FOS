odoo.define('mrp.mrp_state', function (require) {
    ...
    var TimeCounter = AbstractField.extend({
        ...
        willStart: function () {
            var self = this;
            var def = this._rpc({
                model: 'sale.order.line.takt',
                method: 'search_read',
                domain: [
                    ['order_line_id', '=', this.record.data.id],
                ...
                if (self.mode === 'readonly') {
                    ...
                    _.each(result, function (data) {
                        self.service_duration += data.date_end ?
        ...
        _startTimeCounter: function () {
            ...
            if (this.record.data.is_user_working) {
                this.timer = setTimeout(function () {
                    self.service_duration += 1000;
                    self._startTimeCounter();
                }, 1000);
            } else {
                clearTimeout(this.timer);
            }
            this.$el.html($('<span>' + moment.utc(this.service_duration).format("HH:mm:ss") + '</span>'));
        },
    });
